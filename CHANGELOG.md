...
## 1.1.12: Proactive per-orphan discussion, default-branch orphan reaping, visible terminal-finalize failures, and a naturally-resolving Opus tier

- **The Brain now offers a guided, per-orphan walkthrough of preserved orphaned worktrees instead of waiting silently for the operator to name ids.** v1.1.11 surfaced preserved orphans to Slack as a flat list and the Brain acted only on the ids the operator named in a reply (`brain/rules/workflow.md`) — there was no natural-language discussion. Now the Brain reads the surfaced set with a new **read-only `list_surfaced_orphans`** tool, can **proactively offer** when the daemon pushes it a `PRESERVED ORPHANS SURFACED` notice carrying the repo(s) + count ("you have N preserved orphan(s) — want to walk through them?", once per surfaced set, no re-nag), and — on the operator's trigger or acceptance — presents each orphan **one at a time** with a category-driven recommendation (squash-merged / merged-on-origin → reap; genuinely-unmerged → keep, or merge-then-reap to land the work; dirty → keep), resolving each only via the unchanged `resolve_orphan` on explicit per-orphan consent. Every existing safety rail is preserved verbatim: tip-bound ids, never-infer-an-id, live-worker `protected_paths`, `merge-then-reap` onto `refs/remotes/origin/HEAD` (never the operator's checkout), and dirty force-discard only on operator-named `dirty` category.
  - **`list_surfaced_orphans` is read-only and live.** New tool chain (`WorkspaceService.listSurfacedOrphans` → `list-surfaced-orphans` CLI verb → `WorkspaceClient` → orchestrator MCP tool) reads the existing `orphan_surface` table, excluding operator-kept (muted) rows AND orphans that no longer exist (branch ref gone AND worktree gone, e.g. removed out-of-band) so the walkthrough never presents an un-resolvable phantom. It never writes; a vanished row is pruned by the reaper sweep (`reapAmbiguousOrphans` now deletes any `orphan_surface` row whose branch and worktree are both gone).
  - `brain/rules/workflow.md` is rewritten from passive-wait to the proactive-offer + operator-trigger + strictly-sequential walkthrough, and the daemon (`main.py` `_surface_preserved_orphans`) pushes a `PRESERVED ORPHANS SURFACED` notice (repo + count, gated on a non-empty needs-review set — any preserved orphan that isn't squash-merged) to the Brain so the proactive offer can actually fire; `README.md` updated. (`workspace-service.ts`, `cli.ts`, `workspace_client.py`, `orchestrator_mcp.py`, `main.py`, `workflow.md`, `README.md`; covered by the workspace-manager `workspace-service.test.ts` / `cli.test.ts` / `tool-dispatch.test.ts` suites and `test_worktree_reaper.py` / `test_workspace_client.py` / `test_orchestrator_mcp.py`.)
- **A terminal finalize that keeps failing is now visible instead of silently "transient" forever.** A dead worker whose terminal `abandon` raised on every daemon tick (~13 days on one worker) left no log line, no operator alert, and a `kill_worker` status the Brain read as success. Now: (1) the swallowed workspace-client exceptions in `_probe_finalization_status` and `_abandon_rescue_worker` (authority + abandon) log at WARNING; (2) `_drive_finalization_recovery` counts consecutive TERMINAL failures in any phase other than `finalization` (e.g. authority / probe / abandon) and, past `FINALIZE_FAILURE_SURFACE_CAP` (3), posts once to Slack + Brain — the worker is left running, never completed or abandoned (non-terminal idle outcomes are not counted); (3) `kill_worker`'s failure status now reads "finalization FAILED (phase=…: …) — worker NOT completed". The seam-owns-completion invariant is unchanged. (`orchestrator_mcp.py`, `main.py`; covered by `test_worker_finalize_release.py` / `test_daemon.py`.)
- **Worktree reaper judges orphan merged-ness against the repository's default branch, not whatever branch the primary checkout happens to be on.** `reapAmbiguousOrphans` used the primary checkout's checked-out branch as its ancestry target, so an operator sitting on a feature branch could get a feature-only orphan reaped (or a `main`-merged one preserved). It now targets the branch `refs/remotes/origin/HEAD` names (new `originHeadBranchRef`) when that branch exists locally; when origin/HEAD is unset, or names a branch with no local ref (a `clone -b` checkout, or a stale origin/HEAD after a remote default-branch rename), it keeps the previous target — the primary checkout's current branch — so a `master`/`trunk` repository without origin/HEAD is never judged against a nonexistent `main`; if that fallback is needed and the primary is detached, the repository's sweep fails safely (nothing reaped). With a usable origin/HEAD a detached primary no longer blocks the sweep (v1.1.11 refused every detached primary) — reaping stays limited to orphans already merged into the default branch. The reaper never advances a ref; orphan merge-then-reap is unchanged and still fails closed when its default target (origin/HEAD's branch, else `main`) does not resolve. (`git.ts`, `workspace-service.ts`, rebuilt `dist/`; covered by `workspace-service.test.ts`, including `clone -b`, stale-origin/HEAD, `master`/`trunk`-without-origin/HEAD, and detached-primary cases.)
- **Opus tier is no longer pinned to `claude-opus-4-8` — it resolves naturally via the bare `opus` alias.** The pin existed because the `opus` alias used to auto-resolve to Opus 5, which was too eager to improvise to follow the brainstorm → plan → execute workflow; natural resolution now lands on Opus 5.5, which is reliable, so the pin's premise is obsolete. Every internal Opus-tier string (`provider_config.EXPECTED_MODELS`, `config.py` defaults, `main.py` / `orchestrator_mcp.py` defaults and Fable-unavailable redirect literals, `fable_availability.py`, `notifications.py`, `brain_client.py` fallbacks) and all three config files (`config/ironclaude.json`, `config/ironclaude.json.example`, `commander/config/ironclaude.json`) now use `opus`, exactly as the sonnet/haiku/fable tiers already do — no explicit version id is named anywhere. The `[1m]` 1M-context beta path is unchanged (`_MODELS_NEEDING_1M_BETA=("opus",)`), so the tier still launches as `opus[1m]`. (`provider_config.py`, `config.py`, `main.py`, `orchestrator_mcp.py`, `fable_availability.py`, `notifications.py`, `brain_client.py`, `config/ironclaude.json`, `config/ironclaude.json.example`, `commander/config/ironclaude.json`, `README.md`; covered by the existing config/grader/brain/advisor suites.) **Upgrade step:** a config copied from a pre-1.1.12 `config/ironclaude.json.example` names `claude-opus-4-8` in four places; change every one to `opus`. (1) `providers.clients.claude.models.opus` — `load_config` requires that block to exactly match the approved model map, so the old value makes the daemon refuse to start with `ProviderConfigError: providers.clients.claude.models must exactly match approved models` (a config without a `providers.clients.claude` block is unaffected). (2) `brain_model`, (3) `grader_model`, and (4) `advisor.advisor_model` — these are accepted as-is, so a leftover `claude-opus-4-8` would silently keep the Brain, grader, and Sonnet-worker advisor on Opus 4.8; also check any `advisor.advisor_models` entries you added.
- Deploy: workspace-manager `dist/` rebuilt (copy into both plugin caches); a Commander restart picks up the daemon-side orchestrator tool, the daemon→Brain push, the rewritten Brain rule, the finalize-failure surface, and the unpinned Opus tier — before restarting, verify `claude --model 'opus[1m]'` is accepted and no `ANTHROPIC_DEFAULT_OPUS_MODEL` / `BRAIN_MODEL` / `GRADER_MODEL` override is set in the daemon env. Reviewed via blind tier-up plan reviews (advisor-remediated where a review found issues) + Fable adversarial end reviews of the assembled release: they found and this release fixes phantom orphans (the liveness filter), a proactive offer that couldn't fire (the `PRESERVED ORPHANS SURFACED` daemon push), vanished `orphan_surface` rows never pruned (a sweep reconcile), a reaper target that judged against a nonexistent `main` or a missing local default branch (the origin/HEAD-resolves check with the primary-branch fallback — kept off merge-then-reap, which advances refs and must fail closed), and an incomplete Opus upgrade step. Per-task code reviews A/B. Suites: workspace-manager vitest 486, commander 3322.

## 1.1.11: Worktree lifecycle hardening (auto-reaper + consent-gated orphan cleanup), concurrent-operation memory efficiency, daemon DB write-resilience, delegated-subagent-by-default plan execution, and finalize-recovery hardening

- **Orphaned managed worktrees and `ironclaude/*` branches are now auto-reaped, safely, with no operator action.** The daemon already reaped worktrees that still had a workspace-manager `assignments` row, but a worktree+branch whose row was gone ("row-less" — the assignments DB was reset, or the worktree predated it) was invisible to every automatic path and leaked forever; `reconcileRepository` computed that "ambiguous" set but by design only *reported* it. That is why orphaned `ironclaude/*` branches accumulated (28 in one repo). Now the Commander daemon's hourly maintenance sweep runs a git-state-driven reaper across every managed repository.
  - **`WorkspaceService.reapAmbiguousOrphans`** (`worker/mcp-servers/workspace-manager/src/workspace-service.ts`) tolerates a worktree whose directory was removed outside git (so it can't abort the sweep -- see the ENOENT-tolerant `canonicalPath` below), then deletes a row-less orphan ONLY when it passes every safety gate: not protected/live, branch tip commit-date older than the reap TTL (24h), a clean worktree, and its branch tip already merged into the integration target. Anything dirty or unmerged is **preserved and surfaced**, never deleted (never-lose-work). Exposed as a new `reap-orphans` CLI verb (`cli.ts`) wrapped by `WorkspaceClient.reap_orphans` (`commander/src/ironclaude/workspace_client.py`).
  - **Daemon wiring** (`commander/src/ironclaude/main.py`): `_run_maintenance` enumerates managed repositories (distinct `(repo, machine)` from the workers table plus local repos from `assignments.worktree_path`) and reaps each in its own try/except (one unreachable repo never aborts the sweep), with live-worker paths passed as `protectedPaths` and transport resolved per group. Preserved-unmerged branches are surfaced to Slack (change-gated) and shown as a standing count in the heartbeat (`notifications.py`).
  - **`canonicalPath` is ENOENT-tolerant** (`git.ts`) so a registered worktree whose directory is gone no longer makes `listWorktrees`/`discoverRepository` throw (this also unblocks the existing DB-driven release path); plus a new `listManagedBranches` enumerates dangling `ironclaude/*` branches. The first post-deploy sweep dogfood-cleans the existing orphan backlog, preserving any unmerged branches for review.
- Deploy: Commander-daemon-side changes need a Commander restart; the workspace-manager `dist/` bundles are rebuilt and must be copied into both plugin caches. Covered by `test_worktree_reaper.py`, `test_daemon.py`, `test_notifications.py`, `test_workspace_client.py`, and the workspace-manager `git.test.ts` / `workspace-service.test.ts` / `cli.test.ts` / `tool-dispatch.test.ts` suites (commander 3209, workspace-manager 386).
- **Consent-gated cleanup for the orphaned worktrees the reaper preserves.** The auto-reaper above deletes only clear-cut orphans and *preserves + surfaces* anything dirty or unmerged; this adds the operator-driven resolution path for that preserved set. The daemon surfaces each preserved orphan to Slack with a tip-bound `id` (changes when the branch tip changes, so a stale consent cannot land), a content `category` (squash-merged / merged-on-origin / genuinely-unmerged / dirty) computed by bounded, read-only, fail-safe merge detection, and its `repository_path`; the operator replies with ids + an action and the Brain calls `resolve_orphan(repository_path, resolutions, worker_id)` (`orchestrator_mcp.py`), threading per-id outcomes back. Consent is **re-validated at resolution time** against the surfaced tip and category — a dirty worktree is force-discarded only when the consent named the `dirty` category and the persisted row still agrees; `merge-then-reap` integrates onto the repo's canonical default branch (`refs/remotes/origin/HEAD`, fallback `main`) via off-ref compare-and-swap, never the operator's checkout, and reports `target-moved` if the target advanced concurrently. Live-worker worktrees are passed through as `protected_paths` and are never resolved out from under a running worker. The tip binding is enforced by deriving the surfaced `id` from `sha256(guid + tip)` and looking it up **id-first** (an `id`+`guid` resolution binds to the id, not the stable guid; a `guid`-only destructive resolution is refused). The blanket repo-wide `git worktree prune` is dropped in favor of a **targeted** `git worktree remove --force` for a registered-but-directory-gone orphan, applied on both the reap and merge-then-reap paths only after the merged/TTL/unprotected gates (never-lose-work; no collateral on unrelated worktrees). Adds `orphan_surface` / `orphan_resolution_audit` (migration 6) and the `target-moved` outcome. (`workspace-service.ts`, `git.ts`, `cli.ts`, `orchestrator_mcp.py`, `main.py`, `notifications.py`, `workspace_client.py`, `commander/src/brain/rules/workflow.md`, `README.md`; covered by `test_worktree_reaper.py`, `test_orchestrator_mcp.py`, `test_notifications.py`, `test_workspace_client.py`, and the workspace-manager `workspace-service.test.ts` / `cli.test.ts` / `git-content-merged.test.ts` / `db.test.ts` suites.)
- **Concurrent-operation memory efficiency — the daemon now sees memory pressure, bounds live-worker count, and the MCP servers self-clean.** Running many operator sessions + Commander (Brain + workers) on a 48 GB Mac hit macOS "out of application memory" (swap exhaustion), not an MCP leak: each session spawns 3 MCP servers and the fork wrapper doubled that to 6 node procs/session, while the spawn gate and heartbeat tracked no swap/pressure and finished workers were left running. An investigation (findings note + Fable consult, code-verified) drove eight fixes:
  - **Pressure-aware spawn gate** — `_check_spawn_preconditions` (`orchestrator_mcp.py`) rejects a spawn when `psutil.swap_memory().used` exceeds `max_swap_used_gb` (8.0) or macOS memory pressure reaches `spawn_block_pressure_level` (default 2 = warn; `sysctl kern.memorystatus_vm_pressure_level`), ADDITIVE to the existing 10%-of-total floor and fail-open on a probe error. The pressure threshold is configurable so a box that idles at warn can gate on critical (4) instead — the local daemon config on the 48 GB dev box sets `spawn_block_pressure_level: 4` and `max_swap_used_gb: 50` (it steady-states at pressure 2 / swap 42.8 GB, and OOM'd near 50.7 GB). The **heartbeat** gains a memory line (available / swap / top-3 RSS). (`main.py`, `notifications.py`, `config.py`)
  - **Idle-worker TTL** (`main.py`): a worker idle past `idle_worker_ttl_seconds` (default 1800; 0 disables) is reaped — integrate/rescue via the finalization seam, then `tmux kill-session` (the only thing that ends a live worker, mirroring the stuck-kill path). Armed on the `.done` marker (which the daemon deletes on delivery), gauged by tmux pane-log mtime, disarmed on fresh activity, deferred while a finalization-recovery holds the integration lock, and aborted if activity lands before the kill — never-lose-work, never reap a worker the Brain is reusing.
  - **MCP servers self-exit on parent death** — all three (`episodic-memory`, `state-manager`, `workspace-manager`) exit on stdin EOF plus a `CLAUDE_PPID` liveness poll (macOS has no `PR_SET_PDEATHSIG`). **The fork wrapper is collapsed to an in-process `import`**, halving node MCP procs/session, preserving build-on-first-run, npm-chatter isolation, and the pending-marker cleanup (SIGTERM/SIGINT/SIGHUP handlers).
  - **Never-pattern-kill cleanup** — the dead `_cleanup_zombie_mcp_processes` (its `os.kill(ppid,0)` never fired for a launchd-reparented macOS orphan) is deleted; `_kill_orphan_brains` now targets the recorded Brain PID (captured *before* `brain.shutdown()` clears it), with `pgrep` log-only. **vitest** fork fan-out is capped (`maxForks` 2) in the three MCP packages.
  - **episodic-memory embeddings are offload-ready** — `IC_EMBEDDING_BACKEND` selects the local Xenova pipeline (default, unchanged) or an Ollama `/api/embeddings` endpoint with a hard local fallback (switching requires a one-time re-index — vectors are same-family but not bit-identical). (`embeddings.ts`)
  - Blind + adversarial reviews caught, pre-ship, that the idle-TTL wouldn't actually reap a live worker, that the heartbeat memory line silently no-op'd on the daemon host, and that the brain-kill ran against an already-cleared PID — all fixed before this commit. Covered by `test_orchestrator_mcp.py`, `test_main_validate.py`, `test_idle_worker_ttl.py`, `test_kill_orphan_brains.py`, `test_daemon.py`, `test_local_grader.py`, `test_mcp_entrypoints.py`, `test_notifications.py`, and the three MCP packages' `parent-death-exit` / `embedding-backend` / vitest-config suites (commander 3273; state-manager 244; workspace-manager 391; episodic-memory 26).
- **Boy-scout cleanup folded in** (from the reaper reviews): `listManagedBranches` refname exactness, `hasNonCleanedAssignment` coverage, `reap-orphans` snake_case payload keys, honest partial-removal reporting (`reapedWorktreeOnly`), and the `__new__`-fixture purist fix (no getattr defaults).
- Deploy (memory-efficiency): Commander restart for the daemon changes; the state-manager + workspace-manager `dist/` bundles and all three `cli/mcp-server-wrapper.js` are rebuilt/updated and must be copied into both plugin caches — the collapsed wrapper + self-exit + embedding backend reach only NEW sessions.
- **Daemon state-DB write-resilience — the long-lived connection no longer wedges under write contention.** The daemon holds one long-lived SQLite connection to the shared state DB in SQLite's default deferred-transaction mode; under concurrent write load a lock timeout left a *leaked open transaction* that then poisoned every later statement with an un-retryable `SQLITE_BUSY_SNAPSHOT`, so the connection stayed wedged (`database is locked`) until a manual restart — blocking the push-request sweep, staleness persistence, prompt-incident resolution, and the events prune. `init_db` now opens the connection with `isolation_level="IMMEDIATE"` so a lock timeout fails at the up-front `BEGIN IMMEDIATE` and opens *no* transaction (no leak can form; WAL + `busy_timeout=5000` preserved, bare SELECTs stay lock-light), and a new `_db_write_with_retry` helper wraps each periodic writer: it rolls back on any failure (clearing a leak / releasing the write lock), retries lock errors with bounded backoff, and re-raises into each caller's existing log-and-skip backstop — **without ever recreating the connection** (`WorkerRegistry` and `OrchestratorTools` cache it, so a reconnect would crash the daemon). Deploy: Commander-daemon-side — a restart picks it up and clears any currently-wedged connection. (`db.py`, `main.py`; covered by `test_db.py`, `test_daemon.py`, `test_push.py`.)
- **Plan execution now defaults to delegated subagents, not inline.** The workflow steered toward running plan tasks inline in the orchestrator — subjective inline triggers in `writing-plans` Step 7, a hardcoded `--mode=inline` next-action in both `writing-plans` and the `professional-mode-guard` hook, an `executing-plans` anti-pattern that forbade delegating codebase-reading tasks, and a Subagent-Discipline rule that said to go inline "when complex enough to risk spirals" — with inline's costs never stated and the review gates never credited. Now **subagent-sequential with a Sonnet (Claude) / Terra (Codex) subagent is the explicit default**; inline is reserved for three narrow objective cases (the task is orchestration itself; a step genuinely can't be captured in a focused prompt; a prior subagent attempt on this task already spiraled). A task needing broad codebase reading routes to an `Explore` subagent, not inline. The docs now state inline's real costs (it burns the orchestrator's own context — risking its own compaction — and spends a top-tier model on subagent-tier work) and credit the per-task code-review / testing-theatre / tier-up gates as what catches subagent drift. The Subagent-Discipline inline bullet is rewritten identically across every in-repo mirror (root `AGENTS.md` and `CLAUDE.md` included) and the activation parity constant, and the contradicting Right-Size parenthetical is realigned. (`worker/skills/writing-plans/SKILL.md`, `worker/skills/executing-plans/SKILL.md`, `worker/hooks/professional-mode-guard.sh`, `.claude/rules/behavioral.md`, `AGENTS.md`, `CLAUDE.md`, `worker/CLAUDE.md`, `commander/CLAUDE.md`, `worker/skills/activate-professional-mode/SKILL.md`, `commander/src/ironclaude/templates/worker_claude_md.md`, `commander/src/ironclaude/templates/worker_agents.md`; covered by `test_activation_client_parity.py`.) Deploy: skill/hook/behavioral changes reach only NEW sessions; the hook needs `make deploy-hooks`.
- **Finalize-recovery hardening — content-size-independent integration equality, a `maxBuffer` deadlock fix, and an operator `reopen_for_edit` recovery path.** A `git` subprocess whose output exceeded Node's default 1 MB `maxBuffer` failed finalization with an opaque `ENOBUFS`, wedging integration. Fixes in `worker/mcp-servers/workspace-manager/src/git.ts`: a shared `GIT_MAX_BUFFER` (64 MB) ceiling on **every** git subprocess spawn plus a `gitBufferOverflowError` helper that turns `ENOBUFS` into a clear, actionable error; and `cumulativeBinaryEffect` (the integration cumulative-effect equality proof) rewritten to be **content-size independent** — it streams `git diff --binary --full-index` to a temp file via the subprocess stdout fd and chunk-hashes it (sha256) instead of buffering the whole diff in memory, so a large-but-legitimate commit no longer just relocates the deadlock to a bigger buffer. Byte-exact equality is preserved across all 8 call sites (`integration.ts`); temp files are always cleaned up.
  - **New `reopen_for_edit` reconcile mode** (`integration.ts`, `cli.ts`, `index.ts`) returns a stuck reviewed-but-unintegrated row to an editable `active` state, preserving work: uncommitted or diverged-committed HEAD is snapshotted to a recovery ref first, the worktree is reset to the frozen reviewed commit, the assignment's carried push disposition is nulled, and the finalization candidate/freeze refs + integration lock are cleared. It **refuses an interrupted-CAS row** (the candidate already landed on the integration target — the crash-recovery shape plain reconcile completes via `markIntegrated`), before any mutation, so it can never strand an already-integrated assignment; it routes the operator to plain reconcile instead. Recovery detects "already landed" by **ancestry** (the candidate is reachable from the current target), so a genuine landed row whose target has since *advanced* is completed by plain reconcile rather than stranded; and the reopen guard refuses only a genuine this-lifecycle landed CAS — one whose candidate descends from the held lock's `expected_target` — so a stale prior-lifecycle leftover candidate, an unresolvable target, or a lock-less row proceeds to a cleaning reopen instead of a false refusal.
  - **Marker-aware daemon surface** (`commander/src/ironclaude/main.py`): the daemon counts finalize failures since the newest integrate/reopen marker (`count_failed_since_marker`), alerts once per marker and re-arms once per new integrate/reopen marker (a single `_finalize_marker_seen` episode key), and bounds retries at `FINALIZE_DRIFT_RETRY_CAP = 3`; `commit_worker` logs `finalize_failed` / `finalize_integrated` / `finalize_reopened` events and `recover_worker_integration` accepts `reopen_for_edit` (`orchestrator_mcp.py`); the SHIP-step wizard offers it as a terminal recovery branch (`commander/src/brain/rules/workflow.md`).
  - Reviewed via blind tier-up plan review + advisor remediation per lineage and a final full-corpus adversarial end review (SOLID). Deploy: workspace-manager `dist/` bundles rebuilt (copy into both plugin caches); a Commander restart picks up the daemon marker surface. Covered by the workspace-manager `integration-cases.ts` / `git.test.ts` suites and `test_daemon.py` / `test_orchestrator_mcp.py` / `test_worker_finalize_release.py` / `test_idle_worker_ttl.py` (workspace-manager vitest 472; commander 3305).

## 1.1.10: Busy ≠ down — three-state validator liveness

- **The validator now distinguishes a busy-but-reachable backend from an unreachable one, and treats "slow" as healthy.** The remote grading box (`amd-halo:8080`) is deliberately busy — it runs all local inference — so high latency is normal, not an outage. Previously a slow response and a dead route both counted the same toward the circuit breaker and both raised the "validator degraded" banner. Now:
  - **Connect budget is decoupled from the inference read budget.** Both grader clients take a fixed short `connect_timeout` (default 3s), independent of whether a fallback is configured — replacing the old `2 if fallback else timeout` rule that let a no-fallback client hang the full read timeout (e.g. 600s) just to discover it couldn't connect. New config-schema keys `connect_timeout_seconds` / `probe_timeout_seconds` / `hook_validation_budget_seconds` are resolved block-then-top like `timeout_seconds` (which is now documented as the inference *read* budget only). (`backend_resolver.py`, `ollama_client.py`, `openai_client.py`, `grader.py`, `worker/config-schema/llm-backend.md`)
  - **The breaker counts unreachable only.** `_attempt` splits the failure path: a connection error / connect-timeout counts toward opening; a **read**-timeout (connected, slow) triggers a cheap non-inference liveness probe (`GET /v1/models` / `/api/version`, bounded by `(connect, probe)` ≈ 6s worst-case, run *outside* the breaker path) — probe reachable ⇒ `record_slow` (busy, resets the URL's failure state, never opens); probe fails ⇒ counts as unreachable. **This supersedes the grader circuit-breaker connect-timeout classification below** (the earlier pass in this same release), which grouped `ConnectTimeout` with read-timeouts; the mapper order is reversed so `ConnectTimeout → OllamaConnectionError` (down) and a plain read-timeout → `OllamaTimeoutError` (busy). (`ollama_client.py`, `openai_client.py`)
  - **The heartbeat gains a non-alarming "validator busy" line** for a reachable-but-slow backend; the ⚠️ "unreachable/degraded" banner (with its 1.1.10 threshold + 120s hysteresis for the unreachable counter, preserved) fires only for genuine unreachability. (`notifications.py`, `main.py`)
- **The fast-lane operator-wait grade no longer starves on a slow box.** `_grade_bounded` abandons a grade that overruns its ~5s wall bound, but the daemon thread it left running used to keep `_grade_in_flight` set until the full inference read timeout expired (~10 min on a busy box), silently disabling the awaiting-operator classifier. `LocalGrader.grade` now accepts a per-call `read_timeout` (a separate client cached on `(config mtime, read_timeout)`, leaving the normal cached client untouched) so the abandoned thread ends near the bound and the flag clears promptly. (`grader.py`, `main.py`)
- **The inert `topic-change-detector` UserPromptSubmit hook is removed.** It made a synchronous LLM call on every prompt during plan execution — killed by Claude Code's ~30s hook cap on a busy box — yet its only output was a `systemMessage`, which reaches the *user*, never the model, so it could not actually trigger `plan-interruption`. Topic-change handling is already covered by the `executing-plans` instructions, the `plan-interruption` skill, and the professional-mode-guard file allowlist. (`worker/hooks/topic-change-detector.sh` deleted; `hooks.json`, `README.md`, `setup-ollama-validation/SKILL.md`, `test-misc-hooks-client-aware.sh`)
- **The shared bash validation transport is bounded.** `call_validation_llm` (used by `task-completion-validator.sh` and `get-back-to-work-impl.sh`) now uses a fixed `--connect-timeout` (default 3s, fallback-independent) and `--max-time = hook_validation_budget_seconds` (default 8s) instead of the 600s inference `timeout_seconds`, so a busy/dead box can't hang a hook. **Note:** `hook_validation_budget_seconds` also bounds the Stop-hook GBTW grade, so hook-side LLM validation degrades to a fast pass-through when the box is busy — intended, given the box is busy by design. (`worker/hooks/plan-validator.sh`)
- Deploy: grader/notifications/main changes are Commander-daemon-side (restart picks them up); hook changes (`plan-validator.sh` + the `topic-change-detector` removal) need `make deploy-hooks`, removal of the deployed `topic-change-detector.sh` copies, and an updated installed-cache `hooks.json`. Covered by `test_backend_resolver.py`, `test_ollama_client.py`, `test_openai_client.py`, `test_grader.py`, `test_notifications.py`, `test_daemon.py`, and `worker/hooks/tests/test-openai-backend.sh` / `test-misc-hooks-client-aware.sh`.

**Grader circuit-breaker hardening (the earlier pass in this release, on which the three-state model builds):**

- **The validator circuit breaker no longer declares a reachable backend "down" on a single blip.** A lone transient transport failure to the grading backend (e.g. `[Errno 65] No route to host` on a flaky Wi-Fi hop to `amd-halo:8080`) used to open the shared `_BREAKERS` circuit immediately and back off up to 300s, and the heartbeat then reported "validator degraded (… endpoint(s) down)" for a backend that was actually up. Three fixes in the Commander-daemon transport (`commander/src/ironclaude/`):
  - **Failure threshold.** `_CircuitBreakerRegistry` now opens a URL's breaker only after `_BREAKER_FAILURE_THRESHOLD = 3` consecutive failures (tracked on `_UrlBreaker.failures`), not the first. Below the threshold the breaker stays closed and `allow()` returns `True` without claiming the single half-open probe slot (an `open_until == 0.0` "never-opened" sentinel guards this), so concurrent callers in strike state are not spuriously refused. `backoff_for()` reads "not tripped" (`None`) below the threshold. Any `record_success` still pops the entry and resets the count. (`ollama_client.py`)
  - **Heartbeat-banner hysteresis.** A new `_UrlBreaker.open_since` timestamp plus `degraded_urls()` (gated by `_BREAKER_DEGRADED_GRACE = 120.0s`, keyed off `open_since`) means the "validator degraded" banner fires only after a breaker has been opened ≥120s ago and still actively failing (in an open cooldown or being probed) — not on a momentary open followed by idle. `open_urls()` (raw open state) and the breaker's protective fast-fail are unchanged; hysteresis affects only the human-facing report. `ollama_degraded_urls()` (read by the heartbeat in `main.py`) now returns the grace-gated set. The banner wording changed from "endpoint(s) down" to "endpoint(s) unreachable/degraded" (`notifications.py`), since a reachable-but-slow backend is not down.
  - **Connect-timeout classification.** `requests.ConnectTimeout` subclasses both `ConnectionError` and `Timeout`; every transport mapper caught `ConnectionError` first, so a connect timeout was mislabeled `OllamaConnectionError` ("unreachable") — and, on a no-fallback client whose connect timeout is the full read timeout, a latent long-hang if it were ever retried. The `except requests.Timeout` arm now precedes `except requests.ConnectionError` in all six mappers (`openai_client.py` `_post_chat`/`_do_get_models`; `ollama_client.py` `_do_chat`/`create_model`/`_do_post`/`_do_get`), routing `ConnectTimeout` to `OllamaTimeoutError` while a plain connection error still maps correctly.
- Per-call retry ("retry like a normal caller") was evaluated and **deferred**: it only helps a true sub-second blip, and analysis of the incident that motivated this work showed a *sustained* Mac→box route failure (a network/DNS issue), not a blip — the tuning above fixes the genuine code defects but is not a substitute for the network fix. This is Commander-daemon-side, so a Commander restart picks it up (no plugin-cache refresh). Covered by `test_ollama_circuit_breaker.py`, `test_ollama_client.py`, `test_openai_client.py`, `test_notifications.py`, and `test_daemon.py`.

**Shared-resource secrets deny-list and worktree-provisioning fixes:**

- **The shared-resource secrets deny-list now scans a shared _directory's_ contents, not just the entry path itself.** Previously the deny-list blocked adding a file that looked like a secret (`.env`, `id_rsa`, …) but sharing a whole directory (e.g. `config/`) sailed through and silently symlinked the secrets inside it. `configure_shared_resources` now walks a directory entry's contents and refuses if any nested file matches the deny-list (soft-overridable with `allow_secret_entries`, and the Brain still asks the operator). (`worker/mcp-servers/workspace-manager/src/git.ts` + `workspace-service.ts`, rebuilt `dist/{index,cli,hook-intent}.js`; `orchestrator_mcp.py`, `commander/src/brain/rules/workflow.md`, `README.md`; `git.test.ts`, `workspace-service.test.ts`, `test_orchestrator_mcp.py`, `test_brain_doc_guards.py`)
- **A nested shared-resource entry now creates its parent directory before symlinking, instead of silently dead-ending.** `linkSharedResources` now `mkdirSync(dirname(target), { recursive: true })` before creating the link, so an entry like `data/models` (whose gitignored parent doesn't exist in a fresh worktree) links correctly rather than failing to appear. (`worker/mcp-servers/workspace-manager/src/git.ts`, rebuilt `dist/{index,cli,hook-intent}.js`; `git.test.ts`)

**Operator tooling and workflow guidance:**

- **`restart-codex` force-quits the ChatGPT app by exact executable path via `pkill` instead of the osascript `quit` command.** The AppleScript quit could hang on a modal or an unresponsive app; the script now targets the exact `ChatGPT.app` executable path (a module constant, never a broad pattern), accepts `pkill` exit status 0 or 1 (matched / no-match), raises on ≥2, and still probes for liveness before relaunching. (`worker/scripts/restart-codex.py`, `commander/tests/test_restart_codex.py`)
- **New behavioral principle 15 — a managed worktree missing shared project data is a blocker to report, never to hand-fix.** When a task needs gitignored project data (model weights, assets, caches) present in the primary checkout but absent from the worktree, the worker reports the exact missing path(s) to the Brain (which provisions them via `configure_shared_resources`) rather than hand-writing an `ln -s`, copying data in, or stalling for the operator to touch the worktree. (`.claude/rules/behavioral.md`)

Deploy: the grader/notifications/main/orchestrator/shadow changes are Commander-daemon-side (a Commander restart picks them up); the hook changes (`plan-validator.sh` + the `topic-change-detector` removal) need `make deploy-hooks`, removal of the deployed `topic-change-detector.sh` copies, and an updated installed-cache `hooks.json`; the workspace-manager `dist/` bundles must be rebuilt and copied into both plugin caches.
