...
- **The Brain walks you through preserved orphaned worktrees, one at a time.** When the daemon surfaces preserved orphans, the Brain now proactively offers a guided walkthrough (once per surfaced set, no re-nag) and — on your trigger or acceptance — presents each orphan with a category-driven **reap / keep / merge-then-reap** recommendation, acting only on your explicit per-orphan consent via the unchanged `resolve_orphan`. It reads the set with a new read-only `list_surfaced_orphans` tool that excludes kept orphans and ones that no longer exist, so it never presents an un-resolvable phantom. This replaces the old surface-a-flat-list-and-wait behavior; every consent / tip-bound-id / dirty-discard safety rail is unchanged.
- **A terminal finalize that keeps failing is now visible.** Previously swallowed workspace-client errors on the abandon path now log at WARNING; a worker whose terminal finalize fails more than 3 cycles in a row in any phase other than `finalization` (e.g. authority / probe / abandon) is surfaced **once** to Slack and the Brain — left running, never completed or abandoned; and `kill_worker`'s failure status now reads "finalization FAILED … worker NOT completed" so it can't be mistaken for success.
- **The orphan reaper judges merged-ness against the repository's default branch.** It now uses the branch `origin/HEAD` names (when that branch exists locally) instead of whatever branch the primary checkout happens to be on. Without a usable origin/HEAD it keeps the primary checkout's branch, and if that checkout is detached the repository's sweep fails safely (nothing reaped). `merge-then-reap` is unchanged.
- **The Opus tier resolves naturally** via the bare `opus` alias (now Opus 5.5) instead of a pinned `claude-opus-4-8` — see [Model Configuration](#model-configuration) below. **Upgrade step:** a config copied from a pre-1.1.12 `config/ironclaude.json.example` still names `claude-opus-4-8` in four places — change every one to `opus`: `providers.clients.claude.models.opus` (the loader requires an exact match to the approved model map and otherwise refuses to start), plus `brain_model`, `grader_model`, and `advisor.advisor_model` (these are accepted as-is, so a leftover pin would silently keep the Brain, grader, and advisor on Opus 4.8). Also check any `advisor.advisor_models` entries you added.

### Earlier — v1.1.11

- **Orphaned managed worktrees are auto-reaped, safely, with no operator action.** A worktree + `ironclaude/*` branch whose workspace-manager row was gone ("row-less") used to be invisible to every automatic path and leaked forever (28 accumulated in one repo). The Commander daemon's hourly sweep now runs a git-state-driven reaper across every managed repository, deleting a row-less orphan only when it passes every safety gate (not protected/live, branch tip commit-date older than the reap TTL, clean worktree, branch tip already merged); anything dirty or unmerged is **preserved and surfaced**, never deleted.
- **Consent-gated cleanup for the orphans the reaper preserves.** Each preserved orphan is surfaced to Slack with a tip-bound `id` (changes when the branch tip changes, so a stale consent cannot land), a content `category` (squash-merged / merged-on-origin / genuinely-unmerged / dirty) from bounded, read-only merge detection, and its repo path; the operator replies with ids + an action (reap / keep / merge-then-reap) and the Brain resolves them, re-validating consent against the surfaced tip at resolution time. `merge-then-reap` integrates onto the repo's canonical default branch via off-ref compare-and-swap — never the operator's checkout — and live-worker worktrees are never resolved out from under a running worker.
- **Concurrent-operation memory efficiency.** Running many operator sessions + Commander on a 48 GB Mac hit macOS swap exhaustion, not an MCP leak. The spawn gate now rejects a spawn under swap/memory-pressure thresholds, the heartbeat reports a memory line, idle workers past a TTL are reaped (integrate/rescue first — never lose work), the three MCP servers self-exit on parent death, and the fork wrapper is collapsed to an in-process import (halving node MCP procs/session).
- **Daemon state-DB write-resilience.** The daemon's long-lived SQLite connection used to wedge (`database is locked`) under concurrent write load — a deferred-mode lock timeout leaked an open transaction that poisoned later writes with an un-retryable `SQLITE_BUSY_SNAPSHOT` until a manual restart. It now opens with `isolation_level="IMMEDIATE"` (a timed-out `BEGIN` opens no transaction, so no leak forms) and the periodic writers self-heal via a rollback-based retry that never recreates the connection.
- **Plan execution defaults to delegated subagents, not inline.** The `writing-plans` / `executing-plans` recommendation, the professional-mode-guard hook's next-action, and the Subagent-Discipline behavioral rule now make **subagent-sequential** with a Sonnet (Claude) / Terra (Codex) subagent the default; inline is reserved for narrow objective cases (orchestration itself, a step that can't be captured in a focused prompt, or a task a prior subagent attempt already spiraled on). The per-task code review, testing-theatre, and tier-up gates catch subagent drift, and inline is noted to cost more (it burns the orchestrator's own context and spends a top-tier model on subagent-tier work).
- **Finalize-recovery hardening.** A `git` subprocess whose output exceeded Node's default 1 MB `maxBuffer` used to fail integration with an opaque `ENOBUFS` and wedge finalization. Every git spawn now shares a 64 MB ceiling, and the integration cumulative-effect equality proof is rewritten to be **content-size independent** — it streams `git diff --binary --full-index` to a temp file and chunk-hashes it (byte-exact, leak-free) instead of buffering the whole diff, so a large-but-legitimate commit no longer just relocates the deadlock. A new operator **`reopen_for_edit`** reconcile mode returns a stuck reviewed-but-unintegrated worker to an editable state (work snapshotted to a recovery ref first), while **refusing** a genuine already-landed interrupted-CAS row — detected by ancestry and the held lock's `expected_target` — so recovery can never strand an already-integrated assignment; the daemon surfaces repeated finalize failures once per integrate/reopen episode.
...
A worktree the daemon can no longer tie to a live worker or assignment row is an orphan. The daemon's reaper auto-resolves the clear-cut cases; what it can't -- preserved-dirty or preserved-unmerged worktrees -- it surfaces to Slack with a tip-bound `id` (changes when the branch tip changes, so a stale consent cannot land), a `category` (squash-merged / merged-on-origin / genuinely-unmerged / dirty), and a `repository_path`.

Cleanup is consent-gated: the Brain never reaps, keeps, or merges without your explicit per-orphan consent. It reads the surfaced set with the read-only `list_surfaced_orphans` tool and can proactively offer to walk you through the orphans one at a time (recommending reap / keep / merge-then-reap per orphan by category), but it acts only on what you confirm. Once you name or confirm an orphan id and an action (reap / keep / merge), the Brain calls the orchestrator MCP tool `resolve_orphan(repository_path, resolutions, worker_id)` -- `resolutions` is `[{"id": ..., "action": "reap"|"keep"|"merge-then-reap", "category": ...}, ...]` -- and threads the per-id outcomes back under your message. Currently-live worker worktrees are computed and passed through automatically as `protected_paths`, so a live worktree is never resolved out from under a running worker. `merge-then-reap` integrates the orphan's work into the repo's canonical default branch (`refs/remotes/origin/HEAD`, fallback `main`) -- never your current checkout. To target a different branch, include `integration_target` (a branch name) in that resolution, e.g. `{"id": "<id>", "action": "merge-then-reap", "integration_target": "release/x"}`. `category` is optional and only meaningful for `reap`: it is the surfaced category (e.g. `dirty`) your own message named or quoted; a `reap` of a worktree that has since become dirty force-removes it (discarding uncommitted work) only when the resolution's `category` is `dirty` and the persisted row still agrees -- otherwise it's refused.

- **Ids are exact, never inferred.** The Brain uses only the ids you named, exactly as surfaced; it never guesses or reaps without a named id.
- **Outcomes:** `reaped`, `kept`, `refused-changed` (tip changed since surfacing, or uncommitted work the consent did not cover -- for a dirty entry re-consent naming the dirty category, otherwise re-surface), `not-surfaced` (unknown id -- also occurs when the id was superseded by a newer surfacing post; use the id from the latest post), `skipped-live` (a live worker owns it), `reaped-worktree-only`, `merged-then-reaped`, `conflict` (merge conflict, left for manual handling), `needs-manual-merge`, `refused-dirty` (merge-then-reap refuses a worktree with uncommitted changes -- commit the work first, or use `reap` to discard it), `target-moved` (the integration target advanced concurrently -- retry the same resolution, the orphan is unchanged), `error`.
- **Destructive actions.** `reap` and `merge-then-reap` both remove the worktree and delete the branch (`merge-then-reap` merges the branch into the integration target first, then deletes it); `keep` mutes the id until its tip changes.
...
> current-latest Opus (as the `sonnet`, `haiku`, and `fable` tiers already do). The tier was
> previously pinned to Opus 4.8 because Opus 5 was too eager to improvise to follow IronClaude's
> structured brainstorm → plan → execute workflow reliably; Opus 5.5 resolves that, so the pin is
> retired. To force a specific build, set `ANTHROPIC_DEFAULT_OPUS_MODEL` (below).
