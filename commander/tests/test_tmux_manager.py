...
def ssh_manager():
    mgr = MagicMock()
    mgr.get_ssh_args.return_value = ["ssh", "-o", "ControlMaster=auto", "kandice"]
    return mgr
...
class TestRunHelper:
    ...
    def test_ssh_prefixed(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        tmux_ssh._run(["tmux", "has-session", "-t", "test"], ssh_host="kandice")
        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "ssh"
        assert "kandice" in cmd
    ...
    def test_ssh_without_manager_falls_back_to_local(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        mgr = TmuxManager(log_dir="/tmp/ic-logs")
        mgr._run(["echo", "hello"], ssh_host="kandice")
...
class TestHasSessionSSH:
    ...
    def test_remote_via_ssh(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        assert tmux_ssh.has_session("test", ssh_host="kandice") is True
        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "ssh"
...
class TestKillSessionSSH:
    ...
    def test_remote_via_ssh(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        assert tmux_ssh.kill_session("test", ssh_host="kandice") is True
        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "ssh"
...
class TestSpawnSessionSSH:
    ...
    def test_remote_uses_remote_log_dir(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        tmux_ssh.spawn_session("w1", "echo hi", ssh_host="kandice",
...
class TestCapturePaneSSH:
    ...
    def test_remote_no_check(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="output\n")
        tmux_ssh.capture_pane("test", ssh_host="kandice")
...
class TestReadLogTailSSH:
    ...
    def test_remote_uses_tail_command(self, mock_run, tmux_ssh):
        ...
        result = tmux_ssh.read_log_tail("test", ssh_host="kandice",
                                        remote_log_dir="/remote/logs")
        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "ssh"
        remote_cmd = cmd[-1]
...
class TestGetLogMtimeSSH:
    ...
    def test_remote_parses_epoch(self, mock_run, tmux_ssh):
        ...
        result = tmux_ssh.get_log_mtime("test", ssh_host="kandice",
                                        remote_log_dir="/remote/logs")
...
class TestFileOperations:
    ...
    def test_file_exists_remote(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        assert tmux_ssh.file_exists("/tmp/ic-logs/test.done", ssh_host="kandice") is True
    ...
    def test_read_file_remote(self, mock_run, tmux_ssh):
        ...
        result = tmux_ssh.read_file("/tmp/file", ssh_host="kandice")
    ...
    def test_write_file_remote(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        result = tmux_ssh.write_file("/tmp/test.txt", "content", ssh_host="kandice")
        assert result is True
    ...
    def test_mkdir_p_remote(self, mock_run, tmux_ssh):
        ...
        mock_run.return_value = MagicMock(returncode=0)
        assert tmux_ssh.mkdir_p("/remote/path", ssh_host="kandice") is True
        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "ssh"
