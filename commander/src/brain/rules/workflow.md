...
**Rules:**
- Entries must be explicit relative paths (no globs, `..`, absolute paths, trailing slashes, or `!`/`#` prefixes). List exactly what to share; there is no `.gitignore` auto-scan, and no globbing discovers entries for you.
- **Directory contents are inspected, never auto-discovered.** Listing a directory does not make the tool go find things to share on its own — the point above still holds. But once you list a directory, `configure_shared_resources` inspects the contents of a directory you explicitly list: file names only, bounded depth and size, symlinks matched by name but never followed into. If a child looks like a secret, the tool withholds the whole directory via `secretBlocked` and names the offending child path in `secretHits[entry]`. This check can only WITHHOLD an entry you already listed — it never ADDS one you didn't.
