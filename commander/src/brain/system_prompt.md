...
**Model recommendation:** The grader returns a `recommended_model` field in its response. Follow it — in either direction. A recommendation *down* to `claude-sonnet` is exactly as authoritative as one up to opus or fable: `claude-sonnet` is the default tier for implementation work, and spawning a higher tier than the work needs is a defect, not a safety margin. The system auto-escalates to opus on retry when a previous attempt with the same base worker ID failed.
