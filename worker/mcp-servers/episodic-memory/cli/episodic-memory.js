...
COMMANDS:
  ...
  repair      Backfill indexing gaps in grown session files (run once after upgrade)
...
EXAMPLES:
  ...
  # Repair indexing gaps (run once after upgrading)
...
async function main() {
  try {
    ...
    switch (command) {
      ...
      case 'repair':
        await runScript(join(distDir, 'repair-cli.js'), args);
        break;
