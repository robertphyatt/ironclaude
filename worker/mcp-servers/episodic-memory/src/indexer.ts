...
export async function indexUnprocessed(concurrency: number = 1, noSummaries: boolean = false): Promise<void> {
  ...
  for (const project of projects) {
    ...
    for (const file of files) {
      ...
      // Check if fully indexed — compare MAX(line_end) to current file line count
      const lastIndexed = db.prepare(
        'SELECT MAX(line_end) as max_line FROM exchanges WHERE archive_path = ?'
      ).get(archivePath) as { max_line: number | null };

      const currentLines = getFileLineCount(sourcePath);
      if (lastIndexed.max_line !== null && lastIndexed.max_line >= currentLines) continue;

      const startFromLine = lastIndexed.max_line ?? 0;
      ...
      // Parse and check — filter to only new exchanges for grown files
      const exchanges = await parseConversation(sourcePath, project, archivePath);
      const newExchanges = exchanges.filter(e => e.lineStart > startFromLine);
      if (newExchanges.length === 0) continue;

      unprocessed.push({ project, file, sourcePath, archivePath, summaryPath, exchanges: newExchanges, startFromLine });
    }
  }
