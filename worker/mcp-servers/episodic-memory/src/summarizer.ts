...
function loadHooksConfig(): HooksConfig | null {
  ...
  try {
    ...
    console.error('Warning: Could not load hooks config:', error);
  }
  return null;
}
...
async function callOllama(prompt: string, config: HooksConfig): Promise<string> {
  ...
  // Try primary URL (2s connect timeout if fallback configured)
  try {
    return await fetchOllama(url, payload, fallbackUrl ? 2000 : timeoutMs);
  } catch (primaryError) {
    if (!fallbackUrl) throw primaryError;
    console.error(`Primary Ollama (${url}) failed, trying fallback (${fallbackUrl}):`, primaryError);
  }

  // Try fallback URL with full timeout
  return await fetchOllama(fallbackUrl!, payload, timeoutMs);
}

async function fetchOllama(url: string, payload: object, timeoutMs: number): Promise<string> {
  ...
  try {
    ...
    if (data.response) {
      try {
        ...
        return parsed.summary || data.response;
      } catch {
...
Summarize what happened in 2-4 sentences. Be factual and specific. Limit your summary to 500 words maximum. Output in <summary></summary> tags.
