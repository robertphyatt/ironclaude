...
function migrateSchema(db) {
  ...
  if (rdExists) {
    const rdColumns = db.prepare(`PRAGMA table_info(registered_designs)`).all();
...
function upsertSession(db, session) {
  ...
  if (existing) {
    ...
    const values = columns.map((k) => session[k]);
...
function getWaveTasks(db, sessionId, waveNumber) {
  if (waveNumber !== void 0) {
    const stmt2 = db.prepare(
      `SELECT * FROM wave_tasks WHERE terminal_session = ? AND wave_number = ? ORDER BY task_id`
    );
    return stmt2.all(sessionId, waveNumber);
  }
...
function upsertWaveTask(db, task) {
  const existing = db.prepare(
    `SELECT id FROM wave_tasks WHERE terminal_session = ? AND task_id = ?`
  ).get(task.terminal_session, task.task_id);
  if (existing) {
    db.prepare(`
      UPDATE wave_tasks
...
function updateWaveTaskStatus(db, rowId, status) {
  db.prepare(
    `UPDATE wave_tasks SET status = ?, updated_at = datetime('now') WHERE id = ?`
  ).run(status, rowId);
  walCheckpoint(db);
}
function insertPlanHistory(db, entry) {
  db.prepare(`
...
function validateProfessionalModeTransition(current, target, actor) {
  if (current === target) {
...
function canTransitionTo(current, target) {
  ...
  if (retreatTargets && retreatTargets.includes(target)) {
    return true;
  }
  return false;
}
function validateWorkflowTransition(current, target, context) {
  if (current === target) {
    return { valid: true, reason: "No change required" };
  }
  const retreatTargets = RETREAT_SOURCES[current];
...
var PlanTaskSchema = external_exports.object({
  ...
  description: external_exports.string(),
  allowed_files: external_exports.array(external_exports.string().min(1)),
  depends_on: external_exports.array(external_exports.number().int()),
  steps: external_exports.array(PlanStepSchema)
});
var PlanJsonSchema = external_exports.object({
  name: external_exports.string().min(1),
...
function executeRetreat(db, sessionId, from, to, reason) {
  ...
  if (from === "executing" && session.plan_json) {
    let planData = null;
    try {
      planData = JSON.parse(session.plan_json);
    } catch {
    }
    if (planData) {
  ...
  if (["plan_ready", "plan_marked_for_use", "final_plan_prep", "executing", "reviewing", "plan_interrupted"].includes(from)) {
    ...
    if (session.plan_json) {
      try {
        const planData = JSON.parse(session.plan_json);
...
var readToolDefinitions = [
  {
    ...
    description: "Returns full plan state for the current session: plan_name, current_wave, workflow_stage, review_pending, circuit_breaker, and summaries of completed/pending wave tasks.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: "Validates prerequisites for transitioning to a target workflow stage. Returns { ready: boolean, reason: string }.",
    inputSchema: {
      type: "object",
      properties: {
        stage: {
  ...
  },
  {
    ...
    description: "Returns whether a design file has been consumed (used to generate a plan).",
    inputSchema: {
      type: "object",
      properties: {
        file: {
  ...
  },
  {
    ...
    description: "Returns previous execution attempts (including retreats) for a given design file.",
    inputSchema: {
      type: "object",
      properties: {
        design_file: {
  ...
  },
  {
    ...
    description: "Get full session state snapshot for post-compaction recovery. Returns workflow_stage, professional_mode, plan_name, plan_goal, current_wave, total_waves, and all wave tasks with status.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  switch (name) {
    ...
    case "check_workflow_ready": {
      ...
      if (targetStage === "plan_ready") {
        ...
        const consumedDesign = db.prepare(
    ...
    case "get_plan_history": {
      const designFile = args.design_file;
      if (!designFile) {
        return {
          content: [
            {
    ...
    case "run_diagnostics": {
      ...
      try {
        const row = db.prepare("SELECT COUNT(*) as cnt FROM sessions WHERE terminal_session = ?").get(resolvedId);
      ...
      try {
        const row = db.prepare("SELECT professional_mode FROM sessions WHERE terminal_session = ?").get(resolvedId);
      ...
      try {
        const row = db.prepare("SELECT workflow_stage FROM sessions WHERE terminal_session = ?").get(resolvedId);
...
var writeToolDefinitions = [
  {
    ...
    description: "Registers a design file for the current session. The session must exist and workflow must be brainstorming or idle.",
    inputSchema: {
      type: "object",
      properties: {
        file: {
  ...
  {
    name: "consume_design",
    description: "Consumes a registered design file. The design must exist and not yet be consumed. Transitions workflow to design_ready.",
    inputSchema: {
      type: "object",
      properties: {
        file: {
  ...
  {
    name: "create_plan",
    description: "Creates a plan from validated plan JSON. Validates the plan schema and dependency graph, computes Wave 1, stores plan and wave tasks. Transitions workflow to plan_ready.",
    inputSchema: {
      type: "object",
      properties: {
        plan_json: {
  ...
  },
  {
    ...
    description: "Transitions from plan_ready to executing. Requires an existing plan.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  {
    name: "get_next_tasks",
    description: 'Returns the next available tasks. If the current wave is fully reviewed, computes the next wave. Returns {status: "complete"} if all tasks are done, or {status: "wave_in_progress", pending: [...]} if the current wave is not yet complete.',
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  {
    name: "submit_task",
    description: "Marks a wave task as submitted for review. The task must be in the current wave and have status in_progress.",
    inputSchema: {
      type: "object",
      properties: {
        task_id: {
  ...
  {
    name: "claim_task",
    description: "Transitions a wave task from pending to in_progress. Must be called before submit_task. The task must be in the current wave and have status pending.",
    inputSchema: {
      type: "object",
      properties: {
        task_id: {
  ...
  },
  {
    ...
    description: "Records an A-F code review grade for the current wave. Called by code-review Skill after task-boundary reviews. GBTW reads the grade to advance or block submitted tasks.",
    inputSchema: {
      type: "object",
      properties: {
        grade: {
  ...
  },
  {
    ...
    description: "Transitions workflow from brainstorming to design_ready. If file is provided, auto-registers and consumes the design. Call at end of brainstorming skill after writing design document.",
    inputSchema: {
      type: "object",
      properties: {
        file: {
  ...
  },
  {
    ...
    description: "Transitions workflow from design_marked_for_use to plan_ready. Call at end of writing-plans skill after writing plan files to disk.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: "Transitions workflow to brainstorming from any valid source state (idle, debugging, execution_complete, plan_interrupted). Uses FORWARD_TRANSITIONS table as single source of truth.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  {
    name: "mark_debugging",
    description: "Transitions workflow to debugging from any valid source state (idle, brainstorming). Uses FORWARD_TRANSITIONS table as single source of truth.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  {
    name: "mark_executing",
    description: "Transitions workflow from reviewing back to executing. Call in executing-plans skill after code-review sub-skill returns.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  {
    name: "retreat",
    description: "Retreats to brainstorming or debugging. Always allowed from design_ready, plan_ready, or executing. Preserves artifacts per retreat logic.",
    inputSchema: {
      type: "object",
      properties: {
        to: {
  ...
  {
    name: "reset_session",
    description: "Clears all execution state (plan_json, wave tasks, plan_name, current_wave), resets workflow to idle, clears review_pending and circuit_breaker. Preserves professional_mode.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: 'Sets the professional mode for the current session. Claude can set "on" (from any state) but cannot set "off" (human-only via hooks). Validates transitions via state machine.',
    inputSchema: {
      type: "object",
      properties: {
        value: {
  ...
  },
  {
    ...
    description: "Toggles session ID display in statusline and state change messages. Updates log_session_ids in ~/.claude/ironclaude-hooks-config.json.",
    inputSchema: {
      type: "object",
      properties: {
        value: {
...
function handleWriteTool(name, args, db, sessionId) {
  ...
  switch (name) {
    ...
    case "register_design": {
      ...
      const validStages = ["brainstorming", "idle", "execution_complete"];
    ...
    case "get_next_tasks": {
      ...
      const validExecStages = ["executing", "reviewing"];
      ...
      let planJson;
      try {
        planJson = JSON.parse(session.plan_json);
      } catch {
      ...
      const completedTaskIds = allWaveTasks.filter((t) => t.status === "review_passed").map((t) => t.task_id);
    ...
    case "submit_task": {
      ...
      if (taskId === void 0 || taskId === null || typeof taskId !== "number") {
      ...
      ).get(resolvedId, taskId, currentWave);
      if (!waveTask) {
        return err(
    ...
    case "record_review_verdict": {
      ...
      const taskBoundary = args.task_boundary;
      ...
      const validReviewStages = ["executing", "reviewing"];
...
`;
    fs4.appendFileSync(ERROR_LOG_PATH, line);
  } catch {
  }
}
